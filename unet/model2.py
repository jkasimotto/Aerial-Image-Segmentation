    ...
    def __init__(
        ...
        for feature in reversed(features):
            self.ups.append(
                nn.ConvTranspose2d(
                    feature * 2,
                    # Set in_channels to 2*out_channels of last down layer because the skip connection concatenates a feature map.
                    feature,
                    kernel_size=2,
                    stride=2
                )
    ...
    def forward(self, x):
        ...
        for down in self.downs:
            ...
            skip_connections.append(
                x)  # We append downsampled layers. The first is the one with the highest resolution (no downsampling), the last will be the lowest resolution (all downsampled)
            x = self.pool(x)
        ...
        for idx in range(0, len(self.ups), 2):
            ...
            if x.shape != skip_connection.shape:
                x = TF.resize(x, size=skip_connection.shape[
                                      2:])  # Only resize along height and width leaving batch_size and channels untouched.
